...
namespace Web3Lib.MessageHandler
{
    public class EVMMessageHandler : IJsonRpcMessageHandler
    {
        ...
        {
            var response = await incomingMessages.DequeueAsync(cancellationToken).ConfigureAwait(false);
            ...
            using(var sequence = new Sequence<byte>())
            {
                ...
                return Formatter.Deserialize(sequence);
            }
        }
        ...
        public async ValueTask WriteAsync(JsonRpcMessage content, CancellationToken cancellationToken)
        {
            ...
            using(var sequence = new Sequence<byte>())
            {
                Formatter.Serialize(sequence, content);
                ...
                {
                    TraceSource.TraceEvent(TraceEventType.Error, (int)TraceEvent.HttpErrorStatusCodeReceived, "Received HTTP {0} {1} response to JSON-RPC request for method \"{2}\".", (int)response.StatusCode, response.StatusCode, contentAsRequest.Method);
                }
